...
# 创建新迁移
python scripts/db_manager.py revision -m "描述变更"
```
...
2. **生成迁移** - `python scripts/db_manager.py revision -m "描述"`
3. **检查脚本** - 查看 `alembic/versions/` 中生成的文件
4. **应用迁移** - `python scripts/db_manager.py upgrade`
...
4. **应用迁移** - `python scripts/db_manager.py upgrade`
5. **验证** - `python scripts/db_manager.py current`
6. **重启服务** - `./scripts/restart_services.sh`
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `init` | 初始化数据库 | `python scripts/db_manager.py init` |
| `check` | 检查数据库连接 | `python scripts/db_manager.py check` |
| `current` | 查看当前版本 | `python scripts/db_manager.py current` |
| `history` | 查看迁移历史 | `python scripts/db_manager.py history` |
| `upgrade` | 升级数据库 | `python scripts/db_manager.py upgrade` |
| `downgrade` | 降级数据库 | `python scripts/db_manager.py downgrade` |
| `revision` | 创建新迁移 | `python scripts/db_manager.py revision -m "msg"` |
| `stamp` | 标记版本 | `python scripts/db_manager.py stamp -r 001` |
| `create` | 创建数据库 | `python scripts/db_manager.py create` |
| `drop` | 删除数据库 | `python scripts/db_manager.py drop` |
| `reset` | 重置数据库 | `python scripts/db_manager.py reset` |
...
| `make db-init` | `python scripts/db_manager.py init` |
| `make db-check` | `python scripts/db_manager.py check` |
| `make db-current` | `python scripts/db_manager.py current` |
| `make db-history` | `python scripts/db_manager.py history` |
| `make db-upgrade` | `python scripts/db_manager.py upgrade` |
| `make db-downgrade` | `python scripts/db_manager.py downgrade` |
| `make db-reset` | `python scripts/db_manager.py reset` |
...
2. 尝试 `python scripts/db_manager.py init`
...
### Q: 如何查看帮助？
```bash
python scripts/db_manager.py -h
```
...
1. 查看相关文档的故障排查部分
2. 检查日志输出
3. 运行 `python scripts/db_manager.py check` 检查连接
